...
def get_position_by_ticker(ticker: str) -> Optional[tinvest.PortfolioPosition]:
    ...
    filtered_posotions = list(filter(lambda x: x.ticker.lower() == ticker.lower(), positions))
    if len(filtered_posotions) == 0:
        return None
    return filtered_posotions[0]
...
def create_limit_order_by_figi(
    ...
    if op_type not in ('Buy', 'Sell'):
        raise ValueError('Operation type must be Sell or Buy with upper-case first latter')
    client = tinvest.SyncClient(**_get_api_params_from_config())
